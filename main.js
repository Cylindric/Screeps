...
module.exports.loop = function() {
    ...
    for (var i in Game.spawns) {
        ...
        map.initialise(spawn.room)
        map.decayWalkData(spawn.room)
        var roomFlag = map.getFlag(spawn.room)
        ...
        var needs = {
            ...
            'courier': couriers.length < popgoals['courier'],
        }

        // See if we want to reserve the spawn energy
        roomFlag.memory.reserveSpawn = (roomFlag.memory.reserveSpawn === undefined) ? false : roomFlag.memory.reserveSpawn;
        var need = false;
        for (var n in needs) {
            if (needs[n]) {
                need = true
        ...
        if (need && map.isSpawnReserved(spawn.room) === false) {
            console.log("Detected missing creeps; reserving spawn...")
            map.reserveSpawn(spawn.room, true);
        } else if (!need && map.isSpawnReserved(spawn.room) === true) {
            console.log("Detected no missing creeps; releasing spawn...")
            map.reserveSpawn(spawn.room, false);
        }
        if (map.isSpawnReserved(spawn.room)) {
            spawn.room.visual.circle(spawn.pos, {
                stroke: '#aa0000',
                fill: '',
                lineStyle: 'dotted',
                opacity: 1,
        ...
        msg += 'Spawn Energy: ' + map.getPowerLevel(spawn.room) + '/' + map.getPowerCapacity(spawn.room);
        console.log(msg);

        // if we have loads of energy and nothing waiting to be built, pop a new upgrader
        if (map.isSpawnReserved(spawn.room) === false) {
            if (map.getPowerLevel(spawn.room) >= map.getPowerCapacity(spawn.room)) {
                console.log("Adding a new Upgrader because we have loads of energy.")
                needs['upgrader'] = true;
            }
        }
        ...
        // Hack in the tower logic here for now
        ...
        _.forEach(towers, function(tower) {
            spawn.room.visual.circle(tower.pos, {
                stroke: '#00aa00',
                fill: '',
                lineStyle: 'dotted',
                opacity: 0.1,
            ...
            // Find the most damaged stuff first
